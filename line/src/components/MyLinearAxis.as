	...
	//modify by Barbery   blog：www.stutostu.com
	package components
	{
		...
		public class MyLinearAxis extends NumericAxis 
		{
				...
				else
				{
					for (i = labelBase; i <= labelTop; i += computedInterval)
					{
						...
						if( Math.abs(roundedValue) >= 100000000 )
							str = int(roundedValue/100000000) + '亿';
						else if( Math.abs(roundedValue) >= 10000 )
							str = int(roundedValue/10000) + '万';
						else
													 ...
													 maxValue:Number):void
			{
				...
				//让最大值为坐标最大值
				var y_topBound:Number =	maxValue *1.01;
				...
				//设置间隔为4条刻度线
				if (isNaN(_userInterval))
					computedInterval = (y_topBound - y_lowerBound)/4;
